...
let test_one = snd (convert_grammar (Expr, awksub_rules) )Expr =
  [[T "("; N Expr; T ")"]; [N Num]; [N Expr; N Binop; N Expr]; [N Lvalue];
   [N Incrop; N Lvalue]; [N Lvalue; N Incrop]]
let test_two = snd (convert_grammar (Num, awksub_rules) )Num = 
...
let test_1 = (parse_prefix human_grammar accept_all ["Mnn"; "Yes"; "Please"; "?"]) =
    Some
    ([(Mouth, [N Sound; T "?"]); (Sound, [N Noise; N Sound]);
     (Noise, [T "Mnn"]); (Sound, [N Word; N Sound]); (Word, [T "Yes"]);
     (Sound, [N Word]); (Word, [T "Please"])],
    [])

let rec contains_noise = function
  | [] -> false
  | (Noise,_)::_ -> true
  | _::rules -> contains_noise rules

let accept_only_noise rules frag =
  if contains_noise rules
    then Some (rules, frag)
  else None

let test_2 = (parse_prefix awkish_grammar accept_empty_suffix ["Yes"]) 
  = None
